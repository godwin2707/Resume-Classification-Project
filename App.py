...
from pdf2image import convert_from_bytes # It helps to see pdf as image 
import pytesseract # It helps to do OCR on text
from PyPDF2 import PdfReader # It helps handle pdf files
...
# To Load trained KNN model
with open(r'D:\Projects\Resume Classifer\KNN.pkl', 'rb') as f:
    model = pickle.load(f)

# To Load vectorizer (like TfidfVectorizer)
with open(r'D:\Projects\Resume Classifer\Tfidf.pkl', 'rb') as f:
    vectorizer = pickle.load(f)

# To Load LabelEncoder to convert numeric predictions to job role names
with open(r'D:\Projects\Resume Classifer\label_encoder.pkl', 'rb') as f:
    le = pickle.load(f)

# Streamlit page configuration for  title and layout
st.set_page_config(page_title="Resume Classifier", layout="centered", initial_sidebar_state="collapsed")
